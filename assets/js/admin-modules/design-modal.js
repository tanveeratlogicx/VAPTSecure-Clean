(function () {
  ...
  const { Button, Icon, Modal, Spinner, ToggleControl } = {
  ...
  const getMappedContent =
    window.VAPTSECURE_getMappedContent ||
    ((obj, _mappingKey, fallbackKey) => (obj ? obj[fallbackKey] : undefined));
  ...
  const HistoryModal = ({ feature, updateFeature, onClose }) => {
    ...
    useEffect(() => {
      apiFetch({
        path: `vaptsecure/v1/features/${feature.key || feature.id}/history`,
      })
        .then((res) => {
          setHistory(res);
          setLoading(false);
    ...
    return el(
      ...
      [
        el(
          "div",
          {
            id: "vapt-history-modal-actions",
            className: "vapt-flex-between",
        ...
        loading
          ? el(Spinner)
          : el("div", { id: "vapt-history-modal-table-wrap" }, [
              history.length === 0
                ? el("p", null, __("No history recorded yet.", "vaptsecure"))
                : el(
                    "table",
                          ...
                          el("tr", { key: i }, [
                            ...
                            el(
                              ...
                              el(
                                "span",
                                {
                                  className: `vapt-status-badge status-${h.old_status}`,
                                },
                                h.old_status,
                              ),
                            ),
                            el(
                              ...
                              el(
                                "span",
                                {
                                  className: `vapt-status-badge status-${h.new_status}`,
                                },
                                h.new_status,
                              ),
                            ),
                            el(
                              "td",
                              null,
                              h.user_name || __("System", "vaptsecure"),
                            ),
                            el("td", null, h.note || "-"),
                          ]),
                        ),
        ...
        el("div", { style: { marginTop: "20px", textAlign: "right" } }, [
          el(
            Button,
            { isPrimary: true, onClick: onClose },
            __("Close", "vaptsecure"),
          ),
        ]),
        confirmState &&
  ...
  const DesignModal = ({
    ...
    const MEANINGFUL_DEFAULT = {
      ...
      enforcement: {
        driver: "hook",
        mappings: { feat_enabled: "your_backend_hook_here" },
      },
      _instructions:
        "Paste the AI-generated JSON here to replace this default.",
    };
    ...
    const getInitialSchema = () => {
      ...
      if (typeof feature.generated_schema === "string") {
        try {
          ...
          if (typeof parsed === "string") {
            ...
            if (
              !doubleParsed ||
              (Array.isArray(doubleParsed) && doubleParsed.length === 0)
            )
              return MEANINGFUL_DEFAULT;
            return doubleParsed;
          }
      ...
      if (
        Array.isArray(feature.generated_schema) &&
        feature.generated_schema.length === 0
      )
        return MEANINGFUL_DEFAULT;
      if (
        typeof feature.generated_schema === "object" &&
        Object.keys(feature.generated_schema).length === 0
      )
        return MEANINGFUL_DEFAULT;
    ...
    const [localImplData, setLocalImplData] = useState(
      feature.implementation_data
        ? typeof feature.implementation_data === "string"
          ? JSON.parse(feature.implementation_data)
          : feature.implementation_data
        : {},
    );
    const [customizationText, setCustomizationText] = useState(
      feature.dev_instruct || "",
    );
    const [isSaving, setIsSaving] = useState(false);
    const [saveStatus, setSaveStatus] = useState(null);

    const [includeProtocol, setIncludeProtocol] = useState(
      feature.include_manual_protocol === undefined ||
        feature.include_manual_protocol === null
        ? true
        : feature.include_manual_protocol == 1,
    );
    const [includeNotes, setIncludeNotes] = useState(
      feature.include_operational_notes === undefined ||
        feature.include_operational_notes === null
        ? true
        : feature.include_operational_notes == 1,
    );

    const [isMultiEnv, setIsMultiEnv] = useState(false);
    const [isAdaptiveDeployment, setIsAdaptiveDeployment] = useState(
      feature.is_adaptive_deployment == 1,
    );
      ...
      const handleGlobalPaste = (e) => {
        if (isHoveringSchema) {
          ...
          if (text) {
            onJsonChange(text);
            setSaveStatus({
              message: __("Content Replaced from Clipboard!", "vaptsecure"),
              type: "success",
            });
      ...
      try {
        ...
        if (e instanceof SyntaxError) {
          setAlertState({
            message: sprintf(
              __(
                "Invalid JSON format: %s. Check for hidden characters or syntax errors.",
                "vaptsecure",
              ),
              e.message,
            ),
          ...
          setAlertState({
            message: sprintf(
              __(
                "Execution Error: %s. Please report this to support.",
                "vaptsecure",
              ),
              e.message,
            ),
    ...
    const handleRemoveConfirm = () => {
      ...
      updateFeature(feature.key || feature.id, {
        ...
        .catch(() => {
          ...
          setIsRemoveConfirmOpen(false);
          setAlertState({
            message: __("Failed to remove implementation.", "vaptsecure"),
          });
        });
    };
...
`;
      ...
      if (normalizedTargets.length > 0) {
        ...
        if (
          ...
          selLower.includes("functions") ||
          selLower.includes("hook") ||
          selLower === "wordpress" ||
          selLower === "php"
        )
        ...
        if (targets.includes(".htaccess")) prioritizedDriver = "htaccess";
          ...
          targets.includes("PHP Hook") ||
          targets.includes("WordPress") ||
          targets.includes("PHP Functions") ||
          targets.includes("WordPress Core")
        )
          prioritizedDriver = "hook";
        ...
        else if (dsLower.includes("hook") || dsLower.includes("php"))
          prioritizedDriver = "hook";
      ...
      if (designPromptConfig) {
        contextJson =
          typeof designPromptConfig === "string"
            ? designPromptConfig
        ...
        const defaultTemplate = {
          design_prompt: {
            ...
            schema_definition: isMultiEnv
              ? "VAPT A+ Client-Ready Multi-Environment Interface Schema v3.2"
              : "WordPress VAPT schema with standardized control fields",
            id: "{{id}}",
            title: "{{title}}",
            ...
            interface_layout: {
              ...
              styling:
                "Standardized cards with subtle shadows and clear hierarchy.",
            },
            automation_context: {
              ai_check_prompt: "{{automation_prompts.ai_check}}",
              ai_schema_fields: "{{automation_prompts.ai_schema}}",
      ...
      let displayInstruct =
        feature.dev_instruct ||
        feature.devInstruct ||
        feature.ai_agent_instructions ||
        "";
      if (!displayInstruct && feature.generated_schema) {
        try {
          const schema =
            typeof feature.generated_schema === "string"
              ? JSON.parse(feature.generated_schema)
      ...
      if (!displayInstruct)
        displayInstruct = "No specific guidelines provided.";

      let referenceCode = "";
      if (feature.code_examples && Array.isArray(feature.code_examples)) {
        referenceCode = feature.code_examples
          .map((ex) => {
      ...
      const rawDesc = getMappedContent(
        feature,
        "description",
        "description",
        fieldMapping,
      );
      const rawSev = getMappedContent(
        feature,
        "severity",
        "severity",
        fieldMapping,
      );
      const rawMethod = getMappedContent(
        feature,
        "test_method",
        "test_method",
        fieldMapping,
      );
      const rawVerif = getMappedContent(
        feature,
        "verification_steps",
        "verification_steps",
        fieldMapping,
      ...
      const rawOwasp =
        getMappedContent(feature, "owasp", "owasp", fieldMapping) ||
        getMappedContent(
          feature,
          "compliance",
          "owasp_mapping",
        ...
        feature.owasp ||
        "";
      const rawRemediation = getMappedContent(
        feature,
        "remediation",
        "remediation",
        fieldMapping,
      );
      const rawScenario = getMappedContent(
        feature,
        "attack_scenario",
        "attack_scenario",
        fieldMapping,
      ...
      const rawCvssScore =
        getMappedContent(feature, "cvss_score", "cvss_score", fieldMapping) ||
        feature.cvss_score ||
        "";
      const rawPlatforms =
        getMappedContent(
      ...
      const rawPlatformImpl =
        ...
        {};
      const rawUiLayout =
        getMappedContent(feature, "ui_layout", "ui_layout", fieldMapping) ||
        feature.ui_layout ||
        {};
      const rawComponents =
        getMappedContent(feature, "components", "components", fieldMapping) ||
        feature.components ||
        [];
      const rawActions =
        getMappedContent(feature, "actions", "actions", fieldMapping) ||
        feature.actions ||
        [];
      ...
      contextJson = replaceAll(
        contextJson,
        "description",
        formatValue(rawDesc) || "None provided",
      );
      contextJson = replaceAll(
        contextJson,
        "severity",
        (typeof rawSev === "object" ? rawSev.level : rawSev) || "Medium",
      );
      contextJson = replaceAll(
        contextJson,
        "remediation",
        formatValue(rawRemediation),
      );
      ...
      contextJson = replaceAll(
        contextJson,
        "testing_specs",
        JSON.stringify(testingSpecs, null, 2),
      );

      const aiInstructions = {
        ...rootAiInstructions,
        ...(feature.root_ai_agent_instructions || {}),
        ...(feature.ai_agent_instructions || {}),
      };
      const globalSettings = {
        ...rootGlobalSettings,
        ...(feature.root_global_settings || {}),
      ...
      const cvssScore =
        rawCvssScore ||
        (typeof rawSev === "object" ? rawSev.cvss_score : "") ||
        "";
      const cvssVector =
        (typeof rawSev === "object" ? rawSev.cvss_vector : "") || "";
      const affectedComponents =
        (typeof rawDesc === "object" ? rawDesc.affected_components : "") || "";
      ...
      const protectionDetails = {
        available_platforms: Array.isArray(rawPlatforms)
          ? rawPlatforms
          : protection.plugin_dependencies || [],
        platform_implementations: rawPlatformImpl,
      };
      contextJson = replaceAll(
        contextJson,
        "protection_details",
        JSON.stringify(protectionDetails, null, 2),
      );

      contextJson = replaceAll(
        contextJson,
        "ui_layout",
        JSON.stringify(rawUiLayout, null, 2),
      );
      contextJson = replaceAll(
        contextJson,
        "components",
        JSON.stringify(rawComponents, null, 2),
      );
      contextJson = replaceAll(
        contextJson,
        "actions",
      ...
      contextJson = replaceAll(
        contextJson,
        "testing_specs",
        JSON.stringify(testingSpecsFull, null, 2),
      );

      contextJson = replaceAll(
        contextJson,
        "verification_engine",
        JSON.stringify(verifEngine, null, 2),
      );
      contextJson = replaceAll(
        contextJson,
        "relationships",
      ...
      contextJson = replaceAll(
        contextJson,
        "performance_impact",
        JSON.stringify(perfImpact, null, 2),
      );

      contextJson = replaceAll(
        contextJson,
        "reporting",
        JSON.stringify(feature.reporting || {}, null, 2),
      );
      contextJson = replaceAll(
        contextJson,
        "references",
        JSON.stringify(feature.references || [], null, 2),
      );
      ...
      contextJson = replaceAll(
        contextJson,
        "previous_schema",
        feature.generated_schema || "None",
      );

      const prompts = feature.automation_prompts || {};
      contextJson = replaceAll(
        contextJson,
        "automation_prompts.ai_ui",
        prompts.ai_ui || "Interactive JSON Schema for VAPT Workbench.",
      );
      contextJson = replaceAll(
        contextJson,
        "automation_prompts.ai_check",
        prompts.ai_check ||
          `PHP verification logic for ${feature.label || "this feature"}.`,
      );
      contextJson = replaceAll(
        contextJson,
        "automation_prompts.ai_schema",
        prompts.ai_schema ||
          `Essential schema fields for ${feature.label || "this feature"}.`,
      );

      const featureSeverity =
        typeof rawSev === "object" ? rawSev : { level: rawSev };
      const businessImpact = featureSeverity.business_impact || "";

      const featureDesc =
        typeof rawDesc === "object" ? rawDesc : { summary: rawDesc };
      ...
      ${
        isMultiEnv
          ? `--- A+ CLIENT-READY REQUIREMENTS (v3.2) ---
      ...
      6. **Client Verification**: Include \`client_verification\` with http-probes and user-friendly messaging.`
      ...
      \`\`\`json
      {
        ...
        "client_deployment": { "profiles": { "auto_detect": { "deploy_order": ["apache_htaccess", "php_functions"] } } },`
            : `"enforcement": {
      ...
      const domainPlaceholders = [
        /https?:\/\/(?:www\.)?(?:domain\.com|yourdomain\.com|example\.com|mysite\.com)/gi,
        /(?:www\.)?(?:domain\.com|yourdomain\.com|example\.com|mysite\.com)/gi,
      ];
      ...
      relativePaths.forEach((regex) => {
        qualifiedPrompt = qualifiedPrompt.replace(
          regex,
          (match, offset, fullText) => {
            const prevChar = fullText.substring(offset - 3, offset);
            if (
              prevChar === "://" ||
              fullText.substring(offset - 7, offset).includes("http")
            )
              return match;
      ...
      const copyToClipboard = (text) => {
        const fallbackCopy = (t) => {
          ...
          return success
            ? Promise.resolve()
            : Promise.reject("ExecCommand Failed");
        };
      ...
      copyToClipboard(personalizedPrompt)
        .then(() => {
          setSaveStatus({
            message: __("Design Prompt copied!", "vaptsecure"),
            type: "success",
          });
          ...
          el(
            ...
            [
              el(
                ...
                null,
                sprintf(
                  __("Design Implementation: %s", "vaptsecure"),
                  feature.label,
                ),
              ),
            ],
      ...
      [
        saveStatus &&
          el(
            ...
            [
              el(Icon, {
                icon: saveStatus.type === "error" ? "warning" : "yes",
                size: 20,
              }),
              saveStatus.message,
            ],
        ...
        el(
          ...
          [
            el("div", { id: "vapt-design-modal-left-col" }, [
              (() => {
                const isAPlus =
                  parsedSchema?.metadata?.schema_grade === "A+" ||
                  parsedSchema?.schema_grade === "A+";
                return el(
                  "div",
                  {
                    id: "vapt-design-modal-actions",
                    className: "vapt-flex-row",
                  },
                  [
                    !isAPlus &&
                      el(
                        Button,
                        {
                          id: "vapt-btn-copy-prompt",
                          className: "vapt-btn-flex-center",
                          isSecondary: true,
                          onClick: copyContext,
                          icon: "clipboard",
                        },
                        __("Copy AI Design Prompt", "vaptsecure"),
                      ),
                    el(
                      Button,
                      {
                        ...
                        onClick: () => {
                          setConfirmState({
                            message: __(
                              "Are you sure you want to reset the schema? This will wash away any changes.",
                              "vaptsecure",
                            ),
                            isDestructive: true,
                            onConfirm: () => {
                              setConfirmState(null);
                              onJsonChange(
                                JSON.stringify(MEANINGFUL_DEFAULT, null, 2),
                              );
                              setSaveStatus({
                                message: __("Schema Reset!", "vaptsecure"),
                                type: "success",
                              });
                              setTimeout(() => setSaveStatus(null), 2000);
                            },
                ...
                return el(
                  ...
                  [
                            ...
                            Button,
                            {
                              ...
                              onClick: () => {
                                if (window.VAPTSECURE_APlusGenerator) {
                                  const tempFeature = {
                                    ...feature,
                                    include_manual_protocol: includeProtocol,
                                    include_operational_notes: includeNotes,
                                  };
                                  const updatedSchema =
                                    window.VAPTSECURE_APlusGenerator.generate(
                                      tempFeature,
                                      customizationText,
                                    );
                                  onJsonChange(
                    ...
                    !isAPlus &&
                      el(
                        "div",
                        { className: "vapt-flex-row", style: { gap: "20px" } },
                        [
                          el(ToggleControl, {
                            label: __(
                              "Enable A+ Client-Ready Multi-Env Mode (v3.2)",
                              "vaptsecure",
                            ),
                            checked: isMultiEnv,
                            onChange: setIsMultiEnv,
                            help: __(
                              "Mandates A+ runtime capability detection and client-ready fallback orchestration.",
                              "vaptsecure",
                            ),
                          }),
                          el(ToggleControl, {
                            label: __(
                              "Enable A+ Adaptive Deployment (v4.0)",
                              "vaptsecure",
                            ),
                            checked: isAdaptiveDeployment,
                            onChange: setIsAdaptiveDeployment,
                            help: __(
                              "Automatically adapts enforcement to server environment (Apache, Nginx, PHP) with universal fallback.",
                              "vaptsecure",
                            ),
                          }),
                        ],
              ...
              el(
                ...
                [
                  el(
                    "label",
                    {
                      id: "vapt-schema-editor-label",
                      className: "vapt-label-uppercase",
                    },
                    __("A+ Adaptive Script (Source JSON)", "vaptsecure"),
                  ),
                  el(
                    "div",
                    {
                      id: "vapt-schema-editor-hint",
                      className: "vapt-text-hint",
                    },
                    __("Hover and Ctrl+V to replace content.", "vaptsecure"),
                  ),
                  el("textarea", {
                  ...
                  el(
                    "div",
                    {
                      id: "vapt-customization-textarea-wrap",
                      className: "vapt-flex-col",
                    ...
                    [
                      el(
                        "label",
                        { className: "vapt-label-uppercase" },
                        __("Workbench Customization Guidance", "vaptsecure"),
                      ),
                      el("textarea", {
                        id: "vapt-customization-textarea",
                        className: "vapt-textarea-custom",
                        placeholder: __(
                          "Enter custom instructions for A+ Adaptive Logic generation...",
                          "vaptsecure",
                        ),
              ...
              (() => {
                ...
                if (!displayInstruct && feature.generated_schema) {
                  try {
                    ...
                    vaptLog.warn(
                      "Failed to extract fallback instructions from schema",
                      e,
                    );
                  }
                }
                ...
                if (!displayInstruct)
                  displayInstruct = __(
                    "No specific development guidance available for this feature transition.",
                    "vaptsecure",
                  );
                  ...
                  [
                    ...
                    el(
                      ...
                      (() => {
                        ...
                        if (
                          !displayInstruct ||
                          typeof displayInstruct !== "string"
                        )
                          return displayInstruct;
            ...
            el("div", { id: "vapt-design-modal-right-col" }, [
              el("div", { className: "vapt-design-modal-preview-header" }, [
                el(
                  "div",
                  { className: "vapt-flex-row", style: { gap: "8px" } },
                  [
                    el(Icon, { icon: "visibility", size: 16 }),
                    el(
                      "strong",
                      { className: "vapt-preview-title" },
                      __("Preview Panel: Effective Protections", "vaptsecure"),
                    ),
                  ],
                ),
                el(
                  "div",
                  { className: "vapt-flex-row", style: { gap: "10px" } },
                  [
                    el(
                      Button,
                      {
                    ...
                    el(
                      Button,
                      { isSecondary: true, isSmall: true, onClick: onClose },
              ...
              el("div", { className: "vapt-design-modal-preview-body" }, [
                (() => {
                  ...
                  return el(
                    "div",
                    {
                      id: "vapt-design-modal-preview-stack",
                      className: "vapt-flex-col",
                    },
                    [
                        ...
                        GeneratedInterface
                          ? el(GeneratedInterface, {
                              feature: {
                                ...
                                implementation_data: localImplData,
                              },
                          ...
                          : el(
                              "p",
                              null,
                              __("Loading Preview Interface...", "vaptsecure"),
                            ),
                      ]),
                    ],
                  );
            ...
            [
              el("div", { style: { padding: "25px", textAlign: "center" } }, [
                ...
                el(
                  ...
                  __("Are you sure? This cannot be undone.", "vaptsecure"),
                ),
                el(
                  "div",
                  {
  ...
  window.VAPTSECURE_HistoryModal =
    window.VAPTSECURE_HistoryModal || HistoryModal;
