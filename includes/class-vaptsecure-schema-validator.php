...
/**
 * VAPTSECURE_Schema_Validator
 *
 * Handles schema validation, sanitization, and analysis for VAPTSecure Clean.
...
class VAPTSECURE_Schema_Validator
{
    /**
    ...
    private static function get_cached_pattern_library()
    {
        if (self::$cached_pattern_library === null) {
            $pattern_lib_path =
                VAPTSECURE_PATH . "data/enforcer_pattern_library_v2.0.json";
            if (file_exists($pattern_lib_path)) {
                self::$cached_pattern_library = json_decode(
                    file_get_contents($pattern_lib_path),
                    true,
                );
            } else {
                self::$cached_pattern_library = [];
            }
        }
    ...
    public static function analyze_enforcement_strategy($schema, $feature_key)
    {
        if (!isset($schema["enforcement"])) {
            ...
            if (isset($schema["client_deployment"]["enforcement"])) {
                $schema["enforcement"] =
                    $schema["client_deployment"]["enforcement"];
            } else {
                $schema["enforcement"] = [
                    "driver" => "hook",
                    "mappings" => [],
            ...
            if (
                ...
                empty($schema["enforcement"]["mappings"]) &&
                isset($schema["client_deployment"]["enforcement"])
            ) {
                $schema["enforcement"] =
                    $schema["client_deployment"]["enforcement"];
            }
        ...
        foreach ($mappings as $key => $value) {
            ...
            $web_server_indicators = array_merge($block_indicators, [
                "location ",
                "proxy_pass",
                "fastcgi_pass",
                "<configuration",
        ...
        if ($needs_htaccess && $driver === "hook") {
            error_log(
                "VAPT Intelligence: Auto-switching driver to 'htaccess' for feature $feature_key based on physical file target.",
            );
            $schema["enforcement"]["driver"] = "htaccess";
            $schema["enforcement"]["target"] =
                $schema["enforcement"]["target"] ?? "root";
        ...
        foreach ($mappings as $key => $value) {
            ...
            if (is_string($value)) {
                ...
                if (
                    isset($value["wp-config.php"]) ||
                    isset($value["wp_config"])
                ) {
                    $needs_config = true;
                } elseif (isset($value["config"])) {
                    $val_to_test = is_array($value["config"])
                        ? $value["config"]["code"] ?? ""
                        : $value["config"];
                }
            }
            ...
            if (
                stripos($val_to_test, "define(") !== false ||
                stripos($val_to_test, "wp-config") !== false
            ) {
                $needs_config = true;
            }
        }
        ...
        if ($needs_config && $driver === "hook") {
            error_log(
                "VAPT Intelligence: Auto-switching driver to 'config' for feature $feature_key based on wp-config constants.",
            );
    ...
    public static function sanitize_and_fix_schema($schema)
    {
        ...
        foreach ($schema["controls"] as $index => &$control) {
            ...
            if (
                empty($control["key"]) &&
                !empty($control["type"]) &&
                !in_array($control["type"], $no_key_types)
            ) {
                // Try to find a meaningful ID
                $base =
                    $control["id"] ?? ($control["component_id"] ?? "control");
                $control["key"] = sanitize_key(
                    $base . "_" . $index . "_" . wp_generate_password(4, false),
                );
            }
        }
    ...
    public static function validate_schema($schema)
    {
        if (!is_array($schema)) {
            return new WP_Error(
                "invalid_schema",
                "Schema must be an object/array",
                ["status" => 400],
            );
        }
        ...
        foreach ($schema["controls"] as $index => $control) {
            ...
            if (empty($control["type"])) {
                return new WP_Error(
                    "invalid_schema",
                    sprintf(
                        'Control at index %d must have a "type" field',
                        $index,
                    ),
                    ["status" => 400],
                );
            ...
            if (
                ...
                return new WP_Error(
                    "invalid_schema",
                    sprintf(
                        'Control at index %d must have a "key" field',
                        $index,
                    ),
                    ["status" => 400],
                );
        ...
        if (isset($schema["enforcement"])) {
            ...
            if (
                $schema["enforcement"]["driver"] === "htaccess" &&
                empty($schema["enforcement"]["target"])
            ) {
                return new WP_Error(
                    "invalid_schema",
            ...
            if (
                isset($schema["enforcement"]["mappings"]) &&
                !is_array($schema["enforcement"]["mappings"])
            ) {
                return new WP_Error(
                    "invalid_schema",
    ...
    public static function validate_implementation_data($data, $schema)
    {
        ...
        if (!is_array($data)) {
            return new WP_Error(
                "invalid_impl_data",
                "Implementation data must be an object/array",
                ["status" => 400],
            );
        }
            ...
            switch ($type) {
                case "toggle":
                    if (
                        ...
                        return new WP_Error(
                            "invalid_type",
                            sprintf("Field %s must be a boolean/toggle", $key),
                            ["status" => 400],
                        );
                    }
                    break;
                ...
                case "password":
                    if ($control["input_type"] === "number") {
                        if (!is_numeric($value)) {
                            return new WP_Error(
                                "invalid_type",
                                sprintf("Field %s must be numeric", $key),
                                ["status" => 400],
                            );
                        }
                        if (
                            isset($control["min"]) &&
                            (float) $value < (float) $control["min"]
                        ) {
                            return new WP_Error(
                                "out_of_range",
                                sprintf(
                    ...
                    if (isset($control["options"])) {
                        ...
                        if (!in_array($value, $valid_values)) {
                            return new WP_Error(
                                "invalid_option",
                                sprintf(
                                    "Field %s contains an invalid option",
                                    $key,
                                ),
                                ["status" => 400],
                            );
