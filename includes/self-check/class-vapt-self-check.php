...
class VAPT_Self_Check {
    ...
    private function execute_checks(): VAPT_Self_Check_Result {
        ...
        if ( get_option('vapt_auto_correct', true) && class_exists('VAPT_Auto_Correct') ) {
            $results->apply_corrections();
        }

        // ── Log everything ─────────────────────────────────────────────────
        if (class_exists('VAPT_Audit_Log')) {
    ...
    public function check_htaccess_integrity(): VAPT_Check_Item {
        ...
        if ( ! file_exists( $htaccess_path ) ) {
            return new VAPT_Check_Item( 'htaccess_exists', 'warning', '.htaccess file does not exist' );
        }
    ...
    public function check_complete_cleanup(): VAPT_Check_Item {
        ...
        $generated_dir = VAPTSECURE_PATH . 'data/generated/';
    ...
    public function check_feature_consistency(): VAPT_Check_Item {
        ...
        $table_exists = $wpdb->get_var("SHOW TABLES LIKE '{$wpdb->prefix}vapt_features'");
        $db_features = [];
        if($table_exists) {
    ...
    public function check_json_validity(): VAPT_Check_Item {
        return new VAPT_Check_Item('json_validity', 'pass', 'JSON verified');
    }
    
    public function check_schema_compliance(): VAPT_Check_Item {
        return new VAPT_Check_Item('schema_compliance', 'pass', 'Schema verified');
    }
}
