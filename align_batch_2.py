...
def align_batch_2():
    ...
    # Batch 2: RISK-031 to RISK-060
    batch_ids = [f"RISK-{str(i).zfill(3)}" for i in range(31, 61)]
    ...
    for risk_id in batch_ids:
        ...
        # --- NGINX ALIGNMENT ---
        if "Nginx" not in risk_schema['available_platforms'] and (".htaccess" in risk_schema['available_platforms'] or "PHP Functions" in risk_schema['available_platforms']):
            ...
            target_path = risk_id
            if "Debug Log" in title: target_path = "debug.log"
            elif "Apache Status" in title: target_path = "/server-status"
            elif "PHP Version" in title: target_path = risk_id # Header hide usually
            elif "WordPress Version" in title: target_path = risk_id # PHP meta hide
            elif "Plugin Version" in title: target_path = risk_id # PHP meta hide
            elif "Login Page" in title: target_path = "wp-login.php"
            elif "Sensitive Files" in title: target_path = "\\.(bak|config|sql|log)$"
            
            nginx_code = f"location ~* {target_path} {{\n    deny all;\n}}"
            if "Version Exposed" in title:
                nginx_code = "server_tokens off;"
            if "Cross-Origin" in title:
                header_name = title.split(' ')[-1]
                nginx_code = f"add_header {header_name} \"same-origin\" always;"
        ...
        if "PHP Functions" not in risk_schema['available_platforms'] and "wp-config.php" not in risk_schema['available_platforms']:
            ...
            if "Version Exposed" in title:
                php_logic = "remove_action('wp_head', 'wp_generator');"
                hook_name = "init"
            elif "Cross-Origin" in title:
                header_name = title.split(' ')[-1]
                php_logic = f"header('{header_name}: same-origin');"
...
if __name__ == "__main__":
    align_batch_2()
    print("Batch 2 aligned successfully.")
